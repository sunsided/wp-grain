...
REM Compress
tar cf - . --exclude=.svn --exclude=*.psd --exclude=*.tgz --exclude=*.tar.gz --exclude=./po --exclude=*.tdl --exclude=*.cmd --exclude=*.bat --exclude=*.sh --exclude=*.py --exclude=%grain_tar%* %grain_feature_excludes% | gzip --best --verbose > %grain_tgz%