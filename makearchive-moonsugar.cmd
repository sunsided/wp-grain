...
REM Compress
tar cf - ../grain --exclude=.svn --exclude=*.psd --exclude=*.tgz --exclude=*.log --exclude=.htaccess --exclude=*.tar.gz --exclude=*.zip --exclude=./po --exclude=*.tdl --exclude=*.cmd --exclude=*.bat --exclude=*.sh --exclude=*.py --exclude=%grain_tar%* %grain_feature_excludes% | gzip --best --verbose > %grain_tgz%